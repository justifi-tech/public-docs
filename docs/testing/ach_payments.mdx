...
:::note
ACH payments are only available for USD processing on US platform accounts. For Canadian dollar payments, card-based payment methods are supported through a Canada platform account.
:::
...
The failure scenarios above run the full return flow, so the simulated payment is charged an ACH return fee and records the same balance transactions as a live return. See [ACH Returns](/paymentMethods/achReturns).
