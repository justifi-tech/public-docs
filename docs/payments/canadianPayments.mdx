...
Balance transactions for CAD payments are not created at payment capture time. They are created when settlements are imported asynchronously. 
There will be a delay between when a payment is captured and when its associated balance transactions appear in the [Balance Transactions API](https://docs.justifi.tech/api-spec#tag/Balance-Transactions).
...
Create refunds the same way as USD - on the JustiFi dashboard, via the [Refund Payment web component](https://docs.justifi.tech/web-components/payment-facilitation/refund-payment) or via the [refund payment API](https://docs.justifi.tech/api-spec#tag/Refunds/operation/CreateRefund):
...
CAD payments can be voided within 25 minutes of the original transaction via the [void payment API](https://docs.justifi.tech/api-spec#tag/Payments/operation/VoidPayment) or by clicking the `Refund` button on the JustiFi dashboard within 25 min of payment transaction.
...
| `refund_processing_fee` | −25 | fee return on the standalone refund only |
