...
Fee data is available via the `application_fee` object on the payment response. The `fees` array will be empty on CAD payments.
...
| Dynamic fees (`fees` array) | Supported | Not supported (empty) |
| `application_fee_amount` | Supported | Not supported |
| Fee data on payment | `application_fee` or `fees` array | `application_fee` only |
