...
CNP transactions occur when the cardholder is not physically present, typically used for online purchases.
...
Payment Method Groups offer a way to easily assign available payment methods for a checkout session 
or to simply filter payment methods by a group.
...
### Generating a Checkout session with a PMG
**TBD**
