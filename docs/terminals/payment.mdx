...
We have outlined how to integrate payment processing via terminals in our [Card Present documentation](https://docs.justifi.tech/api-spec#tag/Terminals).
...
Attempting to create a payment with a `card_present` payment method token returns a `400 Bad Request` with the error code
`card_present_payment_method_token_not_supported`. To charge the customer again, collect a new payment method. 
