...
The short version is: [Create a checkout](https://docs.justifi.tech/api-spec#tag/Checkouts/operation/CreateCheckout). Pick a terminal and use the [Pay Terminal API](https://docs.justifi.tech/api-spec#tag/Terminals/operation/payTerminal) 
...
- The credit card information collected for a terminal payment is stored in a payment method record that can currently not be used for other payments. 
The payment method is considered "single use" and has the `payment_method_type` `card_present`.
