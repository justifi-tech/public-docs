...
| Checkout (pre-release) | Checkout create      | write:checkout:`checkout_id`                                |
...
1. Generate Access Token [API](https://developer.justifi.ai/tag/API-Credentials#operation/CreateAccessToken)
...
2. Get the access_token and print to the console
...
3. List Payments [API](https://developer.justifi.ai/tag/Payments#operation/ListPayments) using the access_token generated above and a test account:
...
4. Get the first payment_id from payment list (= latest payment) and print to the console
...
5. Generate Web Component Token [API]

```bash
WC_TOKEN_RESPONSE=$(curl -sS --request POST \
  --url https://api.justifi.ai/oauth/sessions \
  --header 'Authorization: Bearer '${ACCESS_TOKEN}'' \
  --header 'Content-Type: application/json' \
  --data '{
  "resources": [
    {
      "role": "read:account",
      "resource_id": "acc_abc1234"
    }
  ]
}')
```

6. Get the web*component_token and print to the console (\_note: this token is good for 60 minutes*)
...
#### Front end code: payment_details.html

_note: replace WEB_COMPONENT_TOKEN and PAYMENT_ID with the values from the terminal script above_
...
Open the file or go to [http://localhost:8000/payment_details.html](http://localhost:8000/payment_details.html), you should see the [payment details web component](https://storybook.justifi.ai/?path=/docs/components-paymentdetails--docs):

![PaymentDetails](../../static/img/wc_payment_details.png)

#### Front end code: payment_list.html

_note: replace WEB_COMPONENT_TOKEN and ACCOUNT_ID with the values from the terminal script above_
...
Open the file or go to [http://localhost:8000/payment_list.html](http://localhost:8000/payment_list.html), you should see the [payment list web component](https://storybook.justifi.ai/?path=/docs/components-paymentslist--docs):
...
3. Create a business entity [API](https://developer.justifi.ai/tag/Business#operation/CreateBusiness) using the access_token generated above:
...
4. Get the business_id from the resopnse and print to the console
...
WC_TOKEN_RESPONSE=$(curl -sS --request POST \
  ...
  "resources": [
    {
      "role": "write:business",
      "resource_id": "biz_abc1234"
    }
  ]
}')
```

6. Get the web_component_token from the response and print to the console
...
_note: replace WEB_COMPONENT_TOKEN and BUSINESS_ID with the values from the terminal script above_
