...
When creating a payment, the fee breakdown may not be included in the synchronous response. Fee calculation happens downstream after the payment is created. **Use [payment events](https://docs.justifi.tech/api-spec#tag/Events/operation/paymentEvent)** (webhooks) to get the finalized fee breakdown, including which configuration was used and the calculated amounts.
:::
