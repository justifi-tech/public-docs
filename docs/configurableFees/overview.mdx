...
- **Required fee types do not accept `effective_end`.** The `processing_ecomm`, `processing_card_present`, `processing_ach`, and `processing_ach_expedited` types cannot have an end date — they must stay active indefinitely. The only way to change rates is to create a new config, which automatically retires the previous one. See [Chaining Configurations](./managingConfigurations.mdx#chaining-configurations) for details.
- **Brand-specific and platform configs can be retired** at any time by POSTing with an `effective_end`. Payments for that brand will fall back to the base processing config. See [Retiring a Brand-Specific Configuration](./managingConfigurations.mdx#retiring-a-brand-specific-configuration) for details.
...
| --- | --- | --- |
| $100 Visa online | `processing_ecomm` | No Visa-specific config exists, falls back to base |
| $100 Amex online | `amex_brand_ecomm` | Amex-specific config exists, replaces base processing fee |
| $100 Mastercard online | `processing_ecomm` | No Mastercard-specific config exists, falls back to base |
...
- **Test mode**: Create Standard Fee Configurations via the [Fee Configurations API](./managingConfigurations.mdx) and process test payments with auto-calculated fees. Use this to validate your setup before going live.
...
:::warning Fees May Not Be in the Create Payment or Complete Checkout Response
When creating a payment, the fee breakdown may not be included in the synchronous response. Fee calculation happens downstream after the payment is created. **Use [payment events](/payments/paymentsApi#payment-events)** (webhooks) to get the finalized fee breakdown, including which configuration was used and the calculated amounts.
:::
...
2. **Update your payment integration.** Remove any `application_fee_amount` parameters from your payment requests. If you use the `application_fee` field on payment records, switch to reading from the `fees` array instead.
