...
| --- | --- | --- |
| `POST` | `/v1/sub_accounts/:id/fee_configurations/:fee_type` | Create a configuration (auto-retires existing) |
...
| `fee_cap_cents` | integer | No | Maximum fee amount in cents. If the calculated fee exceeds this, the cap is used instead |
| `effective_start` | datetime | No | When the configuration takes effect. Defaults to immediately |
| `effective_end` | datetime | No | When the configuration expires. `null` means it stays active indefinitely |

## Creating a Configuration

To set up fees for a sub-account, POST to the fee type to configure.
...
There is no update endpoint. To change a fee rate, POST a new configuration for the same fee type. The previous configuration is **automatically retired** — its `effective_end` is set to the new configuration's `effective_start`.
...
To schedule a rate change in advance, set `effective_start` to a future date:
...
:::warning Required Fee Types Must Always Have Coverage
The following fee types are **required** once configured — the chain must always end with a config that has no `effective_end`:
...
If you try to create a config with an `effective_end` for a required fee type and there's no subsequent config scheduled to take over, the API will reject the request. Without an active processing configuration, the sub-account cannot process payments of that type.
:::
