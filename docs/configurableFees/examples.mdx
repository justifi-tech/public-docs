...
## Walkthrough: Setting Up a Sub-Account
...
Create a payment with no fee parameters:
...
The fee breakdown may not be included in the create payment response. Fee calculation happens downstream. Subscribe to payment events (webhooks) to receive the finalized fee details.
:::
...
For the same $100 amount with a Visa card:
...
Any auto-calculated fee can be overridden by passing explicit fees in the payment request. This is useful for one-off adjustments — for example, waiving the platform fee on a specific payment.
...
In this example, the processing fee is still auto-calculated from the configuration, but the platform fee is explicitly set to $0. Only the fee types you include in the `fees` array are overridden — everything else is calculated normally.
...
Brand-specific configs only apply to their specific brand. Every other card brand still needs a base processing config. Always set up your base configs (`processing_ecomm`, `processing_card_present`) first.
