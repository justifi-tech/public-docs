...
Checkouts support flexible fee structures that allow you to specify different fee types for each payment method.
This enables transparent reporting for merchants and selective fee refunds.

#### Using the Fees Array (Recommended)

Use the `fees` parameter to specify separate fee types for card and bank account payments:
...
#### Application Fees (Deprecated)

:::caution Deprecated
The `application_fees` parameter is deprecated. Use the `fees` parameter instead for more granular control over fee types and selective refunds.
:::
